...
public class CareerRepositoryImpl implements CareerRepository {
	...
	public List<EnrolledDTO> getStudentsByCarrerAndCity(String careerName, String city) {
		// this.em.getTransaction().begin();
		String jpql = "SELECT new dto.EnrolledDTO (c.id, c.name, s.name, s.city) FROM Student s JOIN StudentCareer sc ON s.dni = sc.student JOIN Career c ON sc.career = c.id WHERE c.name = :careerName AND s.city = :city";
